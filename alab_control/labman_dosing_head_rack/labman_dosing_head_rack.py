...
class DosingHeadRack:
    ...
    def update_motor_config(self):
        ...
        """
        self.set_axis_parameter(AxisParameters.MAX_POSITIONING_SPEED, 750)
        ...
        self.set_axis_parameter(AxisParameters.REF_SEARCH_SPEED, 500)
    ...
    def reference_search(self, timeout: float = None):
        ...
        while True:
            ...
            if timeout is not None and time.time() - start_time > timeout:
                ...
                raise TimeoutError("Timeout while waiting for the reference search to be completed."
                                   " Stopping the motor.")

    def move_to_position(self, position: int, block: bool = True, timeout: float = None):
        """
        ...
        if block:
            ...
            while True:
                ...
                if timeout is not None and time.time() - start_time > timeout:
                    self.stop()
                    raise TimeoutError("Timeout while waiting for the target position to be reached."
                                       " Stopping the motor.")
                time.sleep(0.1)
    ...
    def get_axis_parameter(self, axis_parameter: AxisParameters) -> int:
        ...
        Returns:
            the axis parameter
        """
        return self.send_command(Commands.GAP, type_number=axis_parameter.value)["value"]
    ...
    def send_command(
        ...
        with Serial(self.port, 9600, timeout=1) as ser:
            ...
            reply_address, module_address, status, command_number, value, checksum = unpack('>BBBBIB', receive_frame)
            if checksum != sum(receive_frame[:-1]) % 256:
                raise ChecksumError("Different checksum detected in the received frame.")
