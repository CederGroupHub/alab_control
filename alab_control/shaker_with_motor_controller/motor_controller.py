...
class PIDController:
    ...
    def __init__(
        ...
        kd: float,
        dt: float,
        integral_contribution_limit: float = 1.0,
    ):
        """
        ...
        self.transfer_function = ctrl.TransferFunction(
            [self.kd, self.kp, self.ki], [1, 0]
        )
...
class Motor:
    ...
    def __init__(
        self, minimum_control_speed: float = 0, maximum_control_speed: float = 0.4
    ):
        """
    ...
    def scale_to_control(self, speed: float) -> float:
        ...
        """
        scaled_speed = (speed - self.minimum_control_speed) / (
            self.maximum_control_speed - self.minimum_control_speed
        )
        if scaled_speed < 0.0:
            return 0.0
    ...
    def scale_to_actual(self, control_speed: float) -> float:
        ...
        """
        return (
            control_speed * (self.maximum_control_speed - self.minimum_control_speed)
            + self.minimum_control_speed
        )
...
class SpeedSensor:
    ...
    def __init__(
        ...
        minimum_measurable_speed: float = 0.0,
        maximum_measurable_speed: float = 50.0,
    ):
        """
    ...
    def scale_to_control(self, speed: float) -> float:
        ...
        Scales the given speed to the control range [0, 1].
        """
        scaled_speed = (speed - self.minimum_measurable_speed) / (
            self.maximum_measurable_speed - self.minimum_measurable_speed
        )
        if scaled_speed < 0.0:
            return 0.0
        elif scaled_speed > 1.0:
    ...
    def _onPositionChange(
        self, encoder, positionChange: float, timeChange: float, indexTriggered: bool
    ) -> None:
        """
        Callback function to handle position changes and
        calculate the current speed.
        ...
        """
        self.current_speed = (
            (positionChange / timeChange) * 1000 if timeChange != 0 else 0
        )
...
class DiscretePlant:
    ...
    def __init__(self, sensor: SpeedSensor, actuator: Motor, dt: float):
        """
    ...
    def set_characteristics(self, order: str = "Second", *args, **kwargs) -> None:
        """
        Sets the characteristics of the plant.
        ...
        if order == "First":
            ...
            K, zeta, wn = args
            self.plant_tf = ctrl.TransferFunction(
                [K * wn**2], [1, 2 * zeta * wn, wn**2]
            )
        else:
            raise ValueError(
                "Unsupported order. Only first-order systems are supported."
            )
    ...
    def get_time_to_steady_state(
        ...
        stop_sensor_after_steady_state: bool = True,
            ...
            while not time_stepped:
                if time.time() - previous_time >= self.dt:
                    ...
                    if len(y) > int(steady_state_wait_duration // self.dt):
                        y_diff = max(
                            y[-int(steady_state_wait_duration // self.dt) :]
                        ) - min(y[-int(steady_state_wait_duration // self.dt) :])
        ...
        if not steady_state:
            ValueError(
                "System is not reaching steady-state after maximum test time. Try increasing the maximum test time."
            )
...
class PIDTuner:
    ...
    def __init__(
        ...
        plant: DiscretePlant,
        dt: float = 0.5,
        Kp_test_setting: dict[str, float] = {
            "initial_Kp": 3.0,
            "Kp_increment": 1.0,
            "max_Kp": 10.0,
        },
        ss_test_setting: dict[str, float] = {
            "steady_state_wait_duration": 5,
            "maximum_time": 100,
            "error_limit": 0.01,
            "cooldown_time": 20,
        },
        osc_test_setting: dict[str, float] = {
            "initial_time": 20,
            "time_increment": 20,
            "maximum_time": 50,
            "cooldown_time": 20,
            "minimum_oscillation_height": 0.01,
        },
    ):
        """
    ...
    def find_system_parameters(self) -> None:
        ...
        if no_PV:
            raise ValueError(
                "System is not getting any PV change from 0. Check the system."
            )
        ...
        if (
            overshoot < 0.05
        ):  # Consider it a first-order system if overshoot is negligible
            ...
            rise_time_index = next(
                i for i in range(len(y)) if y[i] >= K * 0.9
            )  # 90% of final value
    ...
    def find_ultimate_gain(self) -> tuple[float, float]:
        ...
        for _ in range(
            ...
            int(self.osc_test_setting["maximum_time"]),
            int(self.osc_test_setting["time_increment"]),
        ):
            maximum_time += test_time
            test_time = test_time + self.osc_test_setting["time_increment"] * len(
                np.arange(
                    self.Kp_test_setting["initial_Kp"],
                    self.Kp_test_setting["max_Kp"],
        ...
        for duration in range(
            int(self.osc_test_setting["initial_time"]),
            int(self.osc_test_setting["maximum_time"]) + 1,
            int(self.osc_test_setting["time_increment"]),
        ):
            for Kp in np.arange(
                self.Kp_test_setting["initial_Kp"],
                    ...
                    while not time_stepped:
                        if time.time() - previous_time >= self.dt:
                            ...
                            self.plant.set_control_output(
                                self.p_controller.update(
                                    self.plant.get_control_PV(), 0.5
                                )
                ...
                if peak_valley_avg:
                    oscillation_amplitudes = [
                        abs(peak_valley_avg - y[peak]) for peak in peaks
                    ]
                    # If sustained oscillations detected, calculate Ku and Pu
                    if all(
                        amp > self.osc_test_setting["minimum_oscillation_height"]
                        for amp in oscillation_amplitudes
                    ):
                        ...
                        break
                time.sleep(
                    self.osc_test_setting["cooldown_time"]
                )  # Cooldown time between tests
            if sustained_oscillation_detected:
                break
        if not self.Ku:
            raise ValueError(
                "Ultimate gain (Ku) and period (Pu) could not be determined."
            )
    ...
    def compute_pid_parameters(self, tuning_type: str = "Custom") -> None:
        ...
        if self.Ku and self.Pu:
            if tuning_type == "Classic":
                ...
                raise ValueError(
                    "tuning_type must be one of 'Classic', 'Some_overshoot', 'No_overshoot','PI'"
                )
            self.Ki = self.Kp / Ti
            self.Kd = self.Kp * Td
        else:
            raise ValueError(
                "Ultimate gain (Ku) and period (Pu) must be determined first."
            )
        if self.controller:
            integral_contribution_limit = self.controller.integral_contribution_limit
            self.controller = PIDController(
                self.Kp, self.Ki, self.Kd, self.dt, integral_contribution_limit
            )
        else:
            self.controller = PIDController(
                self.Kp, self.Ki, self.Kd, self.dt
            )  # use default integral_contribution_limit if PIDController is not initialized yet
...
class DiscreteSetpointProfile:
    ...
    def __init__(
        self, time_points: List | np.ndarray, setpoint_values: List | np.ndarray
    ):
        """
        ...
        if len(time_points) != len(setpoint_values):
            raise ValueError(
                "time_points and setpoint_values must have the same length."
            )
        # check that time_points are in increasing order
        if not all(
            time_points[i] <= time_points[i + 1] for i in range(len(time_points) - 1)
        ):
        ...
        self.interpolated_time = np.arange(
            time_points[0],
            time_points[-1] + np.min(np.diff(time_points)),
            np.min(np.diff(time_points)),
        )
        self.interpolated_setpoints = np.interp(
            self.interpolated_time, time_points, setpoint_values
        )
...
class MotorController:
    ...
    def __init__(self, dt: float = 0.1):
        ...
        self.pid_tuner = PIDTuner(
            ...
            osc_test_setting={
                ...
                "maximum_time": 50,
                "cooldown_time": 2.0,
                "minimum_oscillation_height": 0.05,
            },
        )
        self.dt = dt
    ...
    def set_controller(self, kp, ki, kd, integral_contribution_limit=1.0):
        self.pid_tuner.controller = PIDController(
            kp, ki, kd, self.dt, integral_contribution_limit
        )

    def set_speed_profile(
        self, time_points: List[float], speed_values: List[float]
    ) -> None:
        """
    ...
    def run_profile(
        ...
        try:
            ...
            t = np.arange(
                self.speed_profile.interpolated_time[0],
                self.speed_profile.interpolated_time[-1] + self.dt,
                self.dt,
            )
            ...
            for i in range(len(t)):  # for each time point in the profile
                ...
                while not time_stepped:
                    ...
                    if time.time() - previous_time >= self.dt:
                        ...
                        self.plant.set_control_output(
                            self.pid_tuner.controller.update(
                                control_PV, control_setpoint
                            )
                        )
                        ...
                        # check all values of temperature for the last steady_state_wait seconds
                        if not steady_state and len(y_control) > int(
                            steady_state_wait_duration // self.dt
                        ):
                            y_diff = max(
                                y_control[-int(steady_state_wait_duration // self.dt) :]
                            ) - min(
                                y_control[-int(steady_state_wait_duration // self.dt) :]
                            )
                            if abs(y_diff) < error_limit:
                        ...
                        if steady_state:
                            if len(y) > int(steady_state_wait_duration // self.dt):
                                y_diff = max(
                                    y_control[
                                        -int(steady_state_wait_duration // self.dt) :
                                    ]
                                ) - min(
                                    y_control[
                                        -int(steady_state_wait_duration // self.dt) :
                                ...
                                if abs(y_diff) > error_limit:
                                    steady_state = False
                                    previous_limit = (
                                        self.get_pid_controller().integral_contribution_limit
                                    )
                                    steady_state_PV = y_control[
                                        -int(steady_state_wait_duration // self.dt) :
                                    ][-1]
                                    ...
                                    self.get_pid_controller().set_integral_contribution_limit(
                                        previous_limit + error * ratio
                                    )
...
class DiscreteSpeedProfileGenerator:
    ...
    def generate_profile(self):
        ...
        for i in range(len(self.speed_list)):
            ...
            new_speed_values = [
                self.speed_values[-1] + self.acceleration * time for time in t
            ]
...
def main():
    generator = DiscreteSpeedProfileGenerator(
        acceleration=50.0, speed_list=[25.0], duration_list=[30.0], dt=0.01
    )
