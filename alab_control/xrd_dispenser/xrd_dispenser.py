...
class XRDPrepController:
    ...
    def initialize_grip(self):
        ...
        self.gripper.rotate(deg=0, speed=5, force=25, check_gripper=False)
    ...
    def rotate_gripper(self, deg, speed, force):
        self.gripper.rotate(deg=deg, speed=speed, force=force, check_gripper=False)

    def gripping(self, open_p):
        self.gripper.open_to(
            speed_percentage=50, force_percentage=100, position=open_p * 10
        )

    def shake_gripper(self, count=2, v_time=800):
        self.shaker.repeat_motor(count, v_time)  # (shaking time, shaking delay)
        logger.info("Shaking finished")
    ...
    def initialize(self):
        ...
        self.linear_rail.initialize(wait=True)
        logger.info("Initiallize finished")
        self.gripping(45)
        self.move_rail(0)
        time.sleep(3)
    ...
    def start_mass_monitoring(self):
        self.stop_mass_thread = False
        self.mass_thread = threading.Thread(
            target=self.mass_reading_thread, daemon=True
        )
    ...
    def dispensing_powder(
        self, amount, max_tries: int = 16, max_time: float = 5, time_stamp=0.02
    ):
        ...
        while retry_count < max_tries:
            ...
            if (
                retry_count % 4 == 0 and retry_count > 0
            ):  # Re-align the powder on to the sieve
                if amount < 250:
            ...
            while time.time() - start_time < max_time:
                with self.lock:
                    if self.current_mass is None or not isinstance(
                        self.current_mass, (int, float)
                    ):
                        print("Error: Invalid mass reading. Stopping process.")
                        self.shaker.stop_motor()
                        self.stop_mass_monitoring()
                        raise RuntimeError(
                            "Mass reading failed. Please check the balance connection."
                        )
                ...
                if remain_mass >= amount - 2:
                    ...
                    with self.lock:
                        final_mass = self.balance.get_mass_in_mg(
                            "SP"
                        )  # Read final mass
                ...
                time.sleep(
                    time_stamp
                )  # processing cycle to prevent CPU processing overheating

            retry_count += 1
            self.shaker.stop_motor()
            logger.info(f"Trying... ({retry_count}/{max_tries})")

            if retry_count < max_tries:
                time.sleep(2) if amount < 220 else time.sleep(5)
            else:
                logger.info("Maximum attempts reached. Stopping process.")
                time.sleep(1)

                with self.lock:
                    final_mass = self.balance.get_mass_in_mg("SP")
                logger.info(f"Final mass of XRD holder: {final_mass}")
