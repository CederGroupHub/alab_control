...
class URRobotSSH:
    ...
    def read_program(
        ...
        file_name: str,
        base: str = "/programs",
        header_file_name: Optional[str] = None,
    ) -> str:
        with self._ssh.open_sftp() as sftp:
    ...
    def write_program(
        self, file_name: str, program_string: str, base: str = "/programs"
    ):
        with self._ssh.open_sftp() as sftp:
    ...
    @staticmethod
    def _parse_variables(variables_xml_str: str) -> dict[str, str]:
        root = ET.fromstring(variables_xml_str)
    ...
    def read_installation_variables(
        self, file_name: str = "default.variables", base: str = "/programs"
    ) -> dict[str, str]:
        """
        Read the installation variables from the UR robot.
        The file is expected to be in JSON format.
        """
        with self._ssh.open_sftp() as sftp:
            with sftp.open((Path(base) / file_name).as_posix(), "r") as f:
                compressed_variables = f.read()
                variables = gzip.decompress(compressed_variables).decode("utf-8")
