...
EXPOSURE_DISTANCE = -8 #value between -25 and +25, measured in mm, from the top of the crucible
EXPOSURE_VOLTAGE = "10000" #value between 01000 and 30000, measured in volts
EXPOSURE_TIME = "15000" #value between 250 and 99999, measured in millli seconds


#constants for safe operation
crucible_exp_dist = 75 #71 is the standard
...
class SamplePrepEnder3(Ender3):
    ...
    STUB = (140, 5, None)
    PREP_EXP = (8, 42, None)
    Z_PREP_EXP = (None, None, 30)
    STANDBY = (15, 15, 15)
    Z_STANDBY = (None, None, 15)
    Z_STUB_PICK0 = (None, None, 5)
    Z_STUB_PICK1 = (None, None, 5)
    Z_STUB_PICK2 = (None, None, 5)
...
if __name__ == "__main__":
    ...
    while True:
        picked_confirm = input(
            "ACTION NEEDED: Manually place the grid on the needle and: C to continue, A to abort: "
        )
        if picked_confirm.lower() == "a":
            ...
            time.sleep(PAUSE)
            picked_confirm = input(
                "ACTION NEEDED: Grid picked? If not, R to try again, C to continue: "
            )
            ...
            r.moveto(*r.HOME)
            picked_confirm = input(
                "Press enter when the grid is properly attached."
            )
            break
    ...
    r.moveto(*r.Z_STUB_PICK0)
    picked_confirm = input(
                "ACTION: Manually pick the grid and press enter"
            )
    send_command("STANDBY")
