...
# for i in range(5,9):
#     robot.run_program(f"Pick_Crucible_LBCS_{i}.urp", block=True)
#     robot.run_program(f"Place_Crucible_TF_F_{i-4}.urp", block=True)
...
while any(status):
    ...
    #Remember to add wait for tube furnaces to open fully
...
for i in range(1,3):
    robot.run_program(f"Pick_Crucible_TF_F_{i}.urp", block=True)
...
# for i in range(1,3):
#     robot.run_program(f"Pick_Crucible_TF_H_{i}.urp", block=True)
#     robot.run_program(f"Place_Crucible_BFTR_{i}.urp", block=True)
